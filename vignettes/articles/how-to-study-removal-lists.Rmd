...
```{r, echo=FALSE}
model <- readRDS(system.file("example/vignette1.rds", package = "ocmse"))
```
...
```{r, echo=FALSE}
oc_estimates <- readRDS(system.file("example/vignette2.rds", package = "ocmse"))
...
After running the simulations, we can gather the relative means and credible
intervals from each replicate and create a quick summary plot. Below, we
stack all results into data frames and make a boxplot of the relative means
(values around 1 indicate estimates close to the true value).
